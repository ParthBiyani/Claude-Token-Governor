...
`/token-limit 50 45` — session budget 50%, ship at 45%.
`/token-limit status` — show account 5h usage, detected plan, session accounting and limits.
`/token-limit on|off|reset` — enable, disable, or reset this session's local counter.
